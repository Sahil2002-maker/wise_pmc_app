...
class MaterialStockFormPage extends StatefulWidget {
  final MaterialStockController controller;
  final bool   isEdit;
  final int?   stockId;

  const MaterialStockFormPage({
    super.key,
  ...
  @override
  State<MaterialStockFormPage> createState() => _MaterialStockFormPageState();
}

class _MaterialStockFormPageState extends State<MaterialStockFormPage> {
  static const Color _accent = Color(0xFF2563EB);

  MaterialStockController get _ctrl => widget.controller;

  Future<void> _save() async {
    final ok = widget.isEdit
        ? await _ctrl.saveEdit(widget.stockId!)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          widget.isEdit
              ? 'Edit Material Stock Register'
              : 'Create Material Stock Register',
  ...
  Widget _buildRow(int index, StockRowState row) {
    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          // Total Received + Used Qty
          Row(
            children: [
              Expanded(
                child: _Field(
                  label:       'Total Received Qty',
                  ctrl:        row.totalReceived,
                  placeholder: '0',
...
class _FieldWithDate extends StatelessWidget {
  final String                label;
  final TextEditingController ctrl;
  final VoidCallback          onTap;

  const _FieldWithDate({
    required this.label,
    required this.ctrl,
    required this.onTap,
  });
